...
create_file('ab.info', '1')
create_file('bldb.info', '1')
...
def enable_auto_bull(_, message):
    with open("info.openme", "r") as file:
        lines = file.readlines()
...
def delab_command(_, message):
    user_id_to_remove = message.reply_to_message.from_user.id
    with open("info.openme", "r") as file:
        lines = file.read().splitlines()
    user_ids = lines[2]
    if str(user_id_to_remove) != user_ids:
...
def spam_message(_, message):
    ...
    if user_id != allowed_user_id:
        client.send_message(message.chat.id, "⛔Вам запрещено использовать эту команду, т.к она только для создателя.")
        return
